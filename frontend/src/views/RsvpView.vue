  ...
  <div class="rsvp-page" dir="rtl">
    ...
    <div class="rsvp-container">
      <div v-if="loading" class="rsvp-card skeleton-wrap">
        <div class="skel skel-h2" /><div class="skel skel-line" /><div class="skel skel-line short" />
      </div>
      ...
      <div v-else-if="loadError" class="rsvp-card error-card">
        <div class="err-icon">💔</div><h2>הקישור לא תקין</h2><p>לא מצאנו הזמנה עם הקוד שסופק.</p>
      </div>
      ...
      <div v-else class="rsvp-card fade-in">
        ...
        <form @submit.prevent="handleSubmit" class="rsvp-form">
          <div v-if="!guestPreFilled" class="form-section">
            ...
            <div class="form-group">
              ...
              <input id="rsvp-name" v-model="form.name" type="text" class="input"
                :class="{ 'input-error': errors.name }" placeholder="הכנס/י את שמך המלא" />
              <span v-if="errors.name" class="form-error">{{ errors.name }}</span>
            </div>
            <div class="form-group">
              <label class="label" for="rsvp-phone">מספר טלפון</label>
              <input id="rsvp-phone" v-model="form.phone" type="tel" class="input" placeholder="05X-XXXXXXX" dir="ltr" />
            </div>
            <div class="form-group">
              ...
              <span class="form-hint">קיבלת קוד אישי? הזן כאן לאישור מהיר</span>
            </div>
          </div>

          <div v-else class="prefilled-banner">
            <div class="pf-icon">👋</div>
            <div><p class="pf-name">שלום, {{ guestData.name }}!</p><p class="pf-hint">אנא אשר/י את הגעתך</p></div>
            ...
            <div v-if="form.rsvpStatus === 'confirmed' || form.rsvpStatus === 'maybe'" class="form-section">
              ...
              <div class="meal-options">
                <button v-for="meal in mealOptions" :key="meal.value" type="button"
                  ...
                  <span class="meal-emoji">{{ meal.emoji }}</span>{{ meal.label }}
                </button>
              </div>
            </div>
          </Transition>
          ...
          <div class="form-section">
            ...
            <textarea v-model="form.message" class="input" rows="3" placeholder="כתוב/י מילה טובה..." />
          </div>

          <div v-if="submitError" class="submit-error">{{ submitError }}</div>
          <button type="submit" class="btn btn-primary btn-lg w-full" :disabled="submitting">
            <span v-if="submitting">שולח...</span><span v-else>שלח אישור הגעה 💌</span>
          </button>
        </form>
      </div>
    </div>
...
const loading = ref(false), loadError = ref(false), submitting = ref(false)
const submitted = ref(false), submitError = ref('')
const coupleData = ref(null), guestData = ref(null)
...
const confirmTitle = computed(() => ({ confirmed: 'נהדר! מחכים לכם', declined: 'תודה שעדכנת/ת!' }[form.value.rsvpStatus] || 'תודה! נציר את מקומך'))
const confirmSub   = computed(() => {
  if (form.value.rsvpStatus === 'confirmed') return `אישרנו את הגעת ${form.value.name || 'האורח/ת'} לחתונה 💕`
  if (form.value.rsvpStatus === 'declined')  return 'חבל שלא תוכלו להגיע, מעריכים את העדכון 💙'
  return 'נשמח לדעת מוקדם יותר ✨'
})
function formatDate(d) { if (!d) return ''; return new Date(d).toLocaleDateString('he-IL', { day: '2-digit', month: 'long', year: 'numeric' }) }
function petalStyle(i) { return { top: `${5+(i*7)%80}%`, right: `${(i*13)%95}%`, fontSize: `${14+(i%4)*6}px`, animationDelay: `${(i*0.4).toFixed(1)}s`, animationDuration: `${6+(i%4)*2}s`, opacity: 0.15+(i%5)*0.04 } }
onMounted(async () => { const code = route.params.code || route.query.code; if (code) await loadByCode(code) })
async function loadByCode(code) {
  ...
  try {
    ...
    if (data.type === 'guest' && data.guest) {
      guestData.value = data.guest; form.value.code = code; form.value.name = data.guest.name || ''
      form.value.phone = data.guest.phone || ''; form.value.numPeople = data.guest.numPeople || 1
...
async function onCodeInput() { const c = form.value.code.trim(); if (c.length >= 5) await loadByCode(c) }
function clearPrefill() { guestData.value = null; form.value.code = ''; form.value.name = ''; form.value.phone = '' }
function validate() {
  errors.value = {}
...
async function handleSubmit() {
  ...
  try {
    const payload = { rsvpStatus: form.value.rsvpStatus, numPeople: form.value.numPeople, mealPref: form.value.mealPref || undefined, message: form.value.message || undefined }
    if (form.value.code) { payload.code = form.value.code } else { payload.name = form.value.name.trim(); payload.phone = form.value.phone.trim() || undefined; payload.coupleToken = coupleToken }
    const res = await fetch('/api/rsvp/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
    const data = await res.json()
    if (!res.ok) { submitError.value = data.message || 'שגיאה בשליחה'; return }
    if (data.couple) coupleData.value = data.couple
    submitted.value = true
  } catch { submitError.value = 'שגיאה בחיבור לשרת' } finally { submitting.value = false }
}
function reset() { submitted.value = false; submitError.value = ''; form.value.rsvpStatus = ''; form.value.message = '' }
</script>

<style scoped>
.rsvp-page { min-height: 100vh; background: linear-gradient(135deg, var(--color-primary-bg) 0%, #FFF0FA 50%, #F5F5F7 100%); display: flex; align-items: center; justify-content: center; padding: var(--space-8) var(--space-4); position: relative; overflow: hidden; }
...
@keyframes floatPetal { 0% { transform: translateY(0) rotate(0deg); opacity: 0; } 10% { opacity: 0.15; } 90% { opacity: 0.15; } 100% { transform: translateY(-60px) rotate(20deg); opacity: 0; } }
