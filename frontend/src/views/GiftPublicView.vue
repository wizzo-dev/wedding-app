<template>
  <div class="gift-public-page" dir="rtl">
    <div class="gift-container">
      <div v-if="loading" class="loading-state"><p>טוען...</p></div>
      <div v-else-if="error" class="error-state"><h2>💔 הדף לא נמצא</h2></div>
      <div v-else-if="data">
        <div class="couple-header">
          <h1>{{ data.couple.name1 }} &amp; {{ data.couple.name2 }}</h1>
          <p>רשימת המשאלות לחתונה שלנו 💕</p>
        </div>
        <div v-if="!data.wishes?.length" class="empty"><p>🎀 הרשימה ריקה</p></div>
      ...
      <Teleport to="body">
        <div v-if="modal.show" class="modal-overlay" @click.self="modal.show = false">
          <div class="modal-box">
            <h2>💌 פרטי תשלום</h2>
            <div v-if="data?.payment?.bitPhone" class="pm-row"><strong>Bit:</strong> {{ data.payment.bitPhone }}</div>
            <div v-if="data?.payment?.bankInfo" class="pm-row"><strong>בנק:</strong> {{ data.payment.bankInfo }}</div>
...
.couple-header { text-align: center; background: linear-gradient(135deg, var(--color-primary), #C9177A); color: #fff; border-radius: var(--radius-2xl); padding: var(--space-8); margin-bottom: var(--space-6); }
...
.wish-msg { font-size: var(--font-size-sm); color: var(--color-text-muted); }
.empty { text-align: center; padding: var(--space-12); }
