  ...
  <div class="settings-view" dir="rtl">
    <header class="page-header">
      <div>
        ...
        <p class="page-subtitle">פרטי החתונה שלכם</p>
      </div>
    </header>
      ...
      <form @submit.prevent="save" class="settings-form">
        <div class="form-row">
          <div class="form-group">
            <label>שם הכלה</label>
            <input v-model="form.name1" type="text" class="form-input" placeholder="שם פרטי" />
          </div>
          <div class="form-group">
            <label>שם החתן</label>
            <input v-model="form.name2" type="text" class="form-input" placeholder="שם פרטי" />
          </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input v-model="form.venue" type="text" class="form-input" placeholder="לדוגמה: גן עדן אירועים" />
        </div>
        <div class="form-group">
          <label>כתובת האולם</label>
          <input v-model="form.venueAddress" type="text" class="form-input" placeholder="רחוב, עיר" />
        </div>

        <!-- RSVP Link -->
        <div class="rsvp-box" v-if="rsvpToken">
...
async function load() {
  try {
    ...
    rsvpToken.value = data.rsvpToken || ''
...
function copyRsvp() {
  navigator.clipboard.writeText(rsvpUrl.value)
  copied.value = true
...
.tab-link { display: block; padding: var(--space-3) var(--space-4); font-size: var(--font-size-sm); font-weight: 600; color: var(--color-text-muted); text-decoration: none; border-bottom: 2px solid transparent; margin-bottom: -2px; transition: all var(--transition-fast); border-radius: var(--radius-sm) var(--radius-sm) 0 0; }
...
.form-input { width: 100%; padding: var(--space-3) var(--space-4); border: 1.5px solid var(--color-border); border-radius: var(--radius-lg); font-family: var(--font); font-size: var(--font-size-sm); color: var(--color-navy); background: var(--color-bg-card); transition: border-color var(--transition-fast); }
