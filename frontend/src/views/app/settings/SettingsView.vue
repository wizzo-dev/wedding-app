    ...
    <div class="settings-card">
      ...
      <form @submit.prevent="save" class="settings-form">
        ...
        <div class="form-actions">
          <button type="submit" class="btn btn-primary" :disabled="saving">{{ saving ? 'שומר...' : '💾 שמור שינויים' }}</button>
        </div>
      </form>
    </div>
  </div>
...
async function load() {
  try {
    ...
    form.value = { name1: data.name1||'', name2: data.name2||'', weddingDate: data.weddingDate ? data.weddingDate.slice(0,10) : '', venue: data.venue||'', venueAddress: data.venueAddress||'' }
...
async function save() {
  ...
  try {
    const res = await fetch('/api/users/profile', { method:'PUT', headers: { Authorization:`Bearer ${auth.token}`, 'Content-Type':'application/json' }, body: JSON.stringify(form.value) })
