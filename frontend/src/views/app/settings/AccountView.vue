...
async function changeEmail() {
  ...
  try {
    ...
    currentEmail.value = emailForm.value.email
...
async function changePassword() {
  if (pwForm.value.new !== pwForm.value.confirm) {
    pwError.value = 'הסיסמאות לא תואמות'; return
  }
  if (pwForm.value.new.length < 8) {
    pwError.value = 'הסיסמה צריכה להיות לפחות 8 תווים'; return
  }
...
@media (max-width: 560px) { .settings-view { padding: var(--space-4); } }
</style>
