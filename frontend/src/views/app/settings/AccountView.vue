  ...
  <div class="settings-view" dir="rtl">
    <header class="page-header">
      <div>
        ...
        <p class="page-subtitle">ניהול חשבון</p>
      </div>
    </header>
    ...
    <div class="settings-card">
      ...
      <form @submit.prevent="changeEmail">
        <div class="form-group">
          ...
          <input :value="currentEmail" disabled class="form-input" />
        </div>
        <div class="form-group">
          <label>אימייל חדש</label>
          <input v-model="emailForm.email" type="email" class="form-input" placeholder="new@example.com" required />
        </div>
        <div class="form-actions">
          <button type="submit" class="btn btn-primary" :disabled="emailSaving">{{ emailSaving ? 'שומר...' : 'עדכן אימייל' }}</button>
        </div>
      </form>
    </div>
    <div class="settings-card" style="margin-top:var(--space-5)">
      ...
      <form @submit.prevent="changePassword">
        ...
        <div class="form-group">
          ...
          <input v-model="pwForm.new" type="password" class="form-input" minlength="8" required />
        </div>
        <div class="form-group">
        ...
        <div class="form-actions">
          <button type="submit" class="btn btn-primary" :disabled="pwSaving">{{ pwSaving ? 'שומר...' : 'שנה סיסמה' }}</button>
        </div>
      </form>
    </div>
  </div>
...
async function changeEmail() {
  ...
  try {
    const res = await fetch('/api/users/account', { method:'PUT', headers: { Authorization:`Bearer ${auth.token}`, 'Content-Type':'application/json' }, body: JSON.stringify({ email: emailForm.value.email }) })
    const data = await res.json()
    if (!res.ok) throw new Error(data.error || 'שגיאה')
    currentEmail.value = emailForm.value.email; emailForm.value.email = ''
    emailMsg.value = '✅ האימייל עודכן בהצלחה!'
...
async function changePassword() {
  ...
  if (pwForm.value.new.length < 8) { pwError.value = 'הסיסמה צריכה להיות לפחות 8 תווים'; return }
  pwSaving.value = true; pwMsg.value = null; pwError.value = null
  try {
    const res = await fetch('/api/users/account', { method:'PUT', headers: { Authorization:`Bearer ${auth.token}`, 'Content-Type':'application/json' }, body: JSON.stringify({ currentPassword: pwForm.value.current, newPassword: pwForm.value.new }) })
    const data = await res.json()
    if (!res.ok) throw new Error(data.error || 'שגיאה')
    pwForm.value = { current:'', new:'', confirm:'' }
    pwMsg.value = '✅ הסיסמה שונתה בהצלחה!'
