  ...
  <div class="cards-view" dir="rtl">
    ...
    <div class="filter-bar">
      ...
      <div class="filter-options">
        <label class="filter-toggle"><input type="checkbox" v-model="showPopularOnly" /><span>⭐ פופולרי</span></label>
        <label class="filter-toggle"><input type="checkbox" v-model="showFreeOnly" /><span>חינמי</span></label>
      </div>
    </div>

    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>טוען תבניות...</p></div>
    <div v-else-if="error" class="error-state">⚠️ {{ error }} <button @click="load" class="btn btn-outline">נסה שוב</button></div>
    ...
    <div v-else class="templates-grid">
      <div v-for="t in filtered" :key="t.id" class="template-card" @click="quickPreview(t)">
        <div class="card-preview" :style="{ background: t.bgColor }">
          ...
          <div class="hover-overlay"><span class="preview-hint">לחץ לתצוגה</span></div>
        </div>
        <div class="card-info">
          <div class="card-top">
      ...
      <div class="preview-modal">
        ...
        <div class="modal-preview" :style="{ background: preview.bgColor }">
          <div class="fp-inner">
            ...
            <div class="fp-date" :style="{ color: preview.primaryColor }">יום שישי, י״ד באדר תשפ״ה<br>14.3.2025 | 19:00</div>
            <div class="fp-venue" :style="{ color: preview.primaryColor, opacity: 0.7 }">אולם נצח, הרצליה</div>
          </div>
        </div>
        <div class="modal-footer">
          <h3 style="color:var(--color-navy);font-size:var(--font-size-base);font-weight:800;margin:0 0 4px">{{ preview.name }}</h3>
          <p style="color:var(--color-text-muted);font-size:var(--font-size-xs);margin:0 0 var(--space-4)">{{ preview.description }}</p>
          <router-link :to="`/app/cards/preview/${preview.id}`" class="btn btn-primary">בחר תבנית זו →</router-link>
        </div>
      </div>
    </div>
  </div>
...
function quickPreview(t) { preview.value = t }
...
.loading-state, .error-state, .empty-state { text-align: center; padding: var(--space-12); display: flex; flex-direction: column; align-items: center; gap: var(--space-3); color: var(--color-text-muted); }
...
.preview-modal { background: white; border-radius: var(--radius-2xl); max-width: 360px; width: 100%; overflow: hidden; box-shadow: var(--shadow-2xl); position: relative; }
.close-btn { position: absolute; top: var(--space-3); left: var(--space-3); background: rgba(255,255,255,0.9); border: none; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; z-index: 5; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-sm); }
