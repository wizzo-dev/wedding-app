<template>
  <div class="preview-view" dir="rtl">
    <div class="preview-toolbar">
      ...
      <button @click="useTemplate" class="btn btn-primary btn-sm">השתמש בתבנית →</button>
    </div>
    ...
    <div v-else-if="template" class="preview-content">
      <div class="preview-side">
        <div class="card-canvas" :style="{ background: template.bgColor }">
          <div class="canvas-inner">
            ...
            <div class="canvas-couple" :style="{ color: template.primaryColor }">עמית ❤ מיכל</div>
            <div class="canvas-invite" :style="{ color: template.accentColor }">מתכבדים להזמינכם לחתונתנו</div>
            <div class="canvas-divider" :style="{ background: template.accentColor }"></div>
            <div class="canvas-date" :style="{ color: template.primaryColor }">יום שישי, י"ד באדר תשפ"ה</div>
            <div class="canvas-time" :style="{ color: template.primaryColor }">14.3.2025 | 19:00</div>
            <div class="canvas-venue" :style="{ color: template.primaryColor, opacity: 0.75 }">אולם נצח, הרצליה</div>
            <div class="canvas-rsvp" :style="{ background: template.primaryColor }">אישור הגעה</div>
          </div>
        </div>
        <div class="template-badges">
          <span v-if="template.premium" class="tgt premium">👑 פרמיום</span>
      ...
      <div class="template-details">
        ...
        <div class="detail-section">
          ...
          <div class="color-row">
            <div class="color-item">
              <span class="cswatch" :style="{ background: template.primaryColor }"></span>
              <span class="cname">ראשי</span>
              <code>{{ template.primaryColor }}</code>
            </div>
            <div class="color-item">
              <span class="cswatch" :style="{ background: template.accentColor }"></span>
              <span class="cname">מבטא</span>
              <code>{{ template.accentColor }}</code>
            </div>
            <div class="color-item">
              <span class="cswatch" :style="{ background: template.bgColor, border: '1.5px solid #e5e7eb' }"></span>
              <span class="cname">רקע</span>
        ...
        <div class="cta-box">
          ...
          <button @click="useTemplate" class="btn btn-primary">השתמש בתבנית {{ template.name }} →</button>
        </div>
      </div>
    </div>
  </div>
...
const LABELS = { coupleName:'שמות הזוג', weddingDate:'תאריך', venue:'מקום', rsvpDate:'אישור הגעה', phone:'טלפון', dresscode:'קוד לבוש', blessings:'ברכות' }
function fieldLabel(f) { return LABELS[f] || f }

async function load() {
  loading.value = true; error.value = null
  try {
    const res = await fetch('/api/cards/templates/' + route.params.id, { headers: { Authorization: 'Bearer ' + auth.token } })
    if (!res.ok) throw new Error('תבנית לא נמצאה')
    template.value = await res.json()
  } catch (e) { error.value = e.message }
  finally { loading.value = false }
}
function useTemplate() { router.push('/app/cards') }
onMounted(load)
</script>
...
.preview-content { display: grid; grid-template-columns: 360px 1fr; gap: var(--space-8); align-items: start; }
.card-canvas { border-radius: var(--radius-2xl); min-height: 460px; display: flex; align-items: center; justify-content: center; box-shadow: 0 20px 40px rgba(0,0,0,0.12); }
...
.canvas-invite { font-size: 0.85rem; margin-bottom: var(--space-4); }
...
.canvas-rsvp { display: inline-block; color: white; padding: 10px 24px; border-radius: 100px; font-size: 0.85rem; font-weight: 700; }
...
.cswatch { width: 24px; height: 24px; border-radius: 50%; display: inline-block; }
.cname { font-size: var(--font-size-xs); color: var(--color-text-muted); }
